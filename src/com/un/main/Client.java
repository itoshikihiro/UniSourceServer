...
public class Client {
	public static void main(String[] args) {
		try {
			...
			//update an profile
			objectWriter.writeObject(new Message(81, new Student("jabigelow@ursinus.edu", null, null, 0, 0, 0, null)));
			objectWriter.flush();
			System.out.println("object has sent to server to let server to test if it can generate pdf resume");

			objectWriter.writeObject(new Message(82, new Student("jabigelow@ursinus.edu", null, null, 0, 0, 0, null)));
			objectWriter.flush();
			System.out.println("object has sent to server to let server to test if it can generate pdf resume");
			...
			try {
				...
				int transLen = 0;
				System.out.println("----Start to receive file<" + fileName + ">,size is<" + fileLength + ">----");
				while(true){
					int read = 0;
					...
					if(read == -1)
						break;
					transLen += read;
					System.out.println("receive percentage:" + 100 * transLen/fileLength + "%...");
					fos.write(sendBytes, 0, read);
					fos.flush();
				}
				System.out.println("----<" + fileName + "> receives successfully-------");
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				if(dis != null)
